...
server <- function(input, output, session) {
  ...
  balancing_res <- DigiCAT:::balancing_server("balancing", 
                   ...
                   covariates = reactive(data_upload_res$covars),
                   survey_weight_var = reactive(data_upload_res$survey_weight_var),
                   cluster_var = reactive(data_upload_res$cluster_var),
                   stratification_var = reactive(data_upload_res$stratification_var),
  ...
  outcome_res <- DigiCAT:::outcome_model_server("outcome_model",  
                       ...
                       estimation_stage_res = reactive(balancing_res$estimation_stage_res),
                       balancing_stage_res = reactive(balancing_res$balancing_stage_res),
                       descriptions = desc_global)
