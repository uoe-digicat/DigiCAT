           ...
           div(style = "display: flex;",
               div(style = "width: 49%;",
                   ...
                   radioButtons(NS(id, "CF_radio"), label = h4("Choose a Counterfactual Approach:"),
                                br(),
                                choices = list("Propensity Matching" = "PM", " Inverse probability of treatment weighting (IPTW)" = "IPTW"))
               ),
...
CF_approach_server <- function(id) {
  ...
  moduleServer(id,
               function(input, output, session) {
                 ...
                 observeEvent(input$prev_CF_btn, {
                   updateTabsetPanel(session = session, inputId = "tabs", selected = "upload")
                 })
